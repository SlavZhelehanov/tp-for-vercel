...
export default function AddPost({ useAllPosts }) {
    ...
    return (
        <div className="p-4 bg-white shadow-md rounded-lg flex gap-4 justify-between text-sm">
            {isAuthenticated
                ? <>
                    <img src={user?.avatar} alt="" width={48} height={48} className="w-12 h-12 object-cover rounded-full" />

                    <div className="flex-1">
                        <form action={submintAction} className="flex flex-col gap-4">
                            <input type="url" name="image" id="" placeholder="Add image url..." className="bg-slate-100 rounded-lg p-2" />
